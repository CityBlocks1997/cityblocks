...
function createDimensionalOverlay() {
    ...
    gridContainer.style.cssText = `
        display: grid;
        gap: 10px;
        max-width: 90vw;  /* Reduced from 95vw - ensures videos don't bleed */
        max-height: 85vh; /* Reduced from 95vh - leaves room for exit button */
    `;
...
function updateDimensionalGrid(videoCount) {
    ...
    if (videoCount === 1) {
        ...
        // 3x3 grid - slightly larger
        gridContainer.style.gridTemplateColumns = 'repeat(3, 1fr)';
        gridContainer.style.gridTemplateRows = 'repeat(3, 1fr)';
        gridContainer.style.width = '70vw';
        gridContainer.style.height = '70vh';
    } else {
        // 4x4 grid (12 videos) - FITS ALL VIDEOS WITHOUT BLEEDING
        gridContainer.style.gridTemplateColumns = 'repeat(4, 1fr)';
        gridContainer.style.gridTemplateRows = 'repeat(4, 1fr)';
        gridContainer.style.width = '80vw';  /* Conservative size ensures all fit */
        gridContainer.style.height = '70vh'; /* Leaves comfortable margin */
    }
}
