...
function toggleBuskerMode() {
    ...
    if (buskerModeActive) {
        ...
        buskerWindows.forEach(windowId => {
...
function processSequenceWithDelays(sequence, index) {
    ...
    if (num >= 1 && num <= 12) {
        // Use window number directly instead of category name
        const windowId = `window-${num}`;
        
        // Add window if not already playing
        if (!buskerWindows.has(windowId)) {
            buskerWindows.add(windowId);
            const windowElement = document.getElementById(windowId);
            const videoElement = windowElement.querySelector('video');
            ...
            console.log(`🔊 Added to layer: Window ${num}`);
        }
        
        // Update status
        document.getElementById('playback-status').textContent = 
            `🎸 Layering ${buskerWindows.size} window${buskerWindows.size !== 1 ? 's' : ''}`;
    }
...
document.getElementById('busker-sequence').addEventListener('keypress', function(e) {
    if (e.key === 'Enter') {
        playBuskerSequence();
    }
});

document.getElementById('custom-sequence').addEventListener('keypress', function(e) {
    if (e.key === 'Enter') {
        playCustomSequence();
    }
});
