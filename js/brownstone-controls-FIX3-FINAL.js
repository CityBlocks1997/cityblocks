...
function createDimensionalOverlay() {
    ...
    gridContainer.style.cssText = `
        display: grid;
        gap: 8px;         /* Reduced from 10px - less wasted space */
        max-width: 85vw;  /* Conservative limit */
        max-height: 80vh; /* Conservative limit - plenty of room for exit button */
        overflow: hidden; /* Prevent videos from bleeding outside grid */
    `;
...
function updateDimensionalGrid(videoCount) {
    ...
    if (videoCount === 1) {
        // Solo video - centered and comfortable size
        gridContainer.style.gridTemplateColumns = '1fr';
        gridContainer.style.gridTemplateRows = '1fr';
        gridContainer.style.width = '45vw';
        gridContainer.style.height = '55vh';
    } else if (videoCount <= 4) {
        // 2x2 grid - comfortable viewing size
        ...
        gridContainer.style.width = '65vw';
        gridContainer.style.height = '65vh';
    } else {
        // 4x4 grid (12 videos) - VERY CONSERVATIVE SIZE - ALL MUST FIT
        gridContainer.style.gridTemplateColumns = 'repeat(4, 1fr)';
        gridContainer.style.gridTemplateRows = 'repeat(4, 1fr)';
        gridContainer.style.width = '70vw';  /* Small enough to ensure all 12 fit */
        gridContainer.style.height = '60vh'; /* Conservative height */
    }
}
